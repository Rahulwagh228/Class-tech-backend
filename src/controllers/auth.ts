...
// =============================================================================
// PATCH /api/v1/auth/password   (auth required, any role)
//
// Lets ANY authenticated user (admin / teacher / student / parent) rotate
// their own password. The row to update is located by req.user.id, so a caller
...
//
// Errors:
//   400 — missing fields / new_password too short / same as old
...
export const changePassword = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (update.rowCount === 0) {
      console.error('changePassword: UPDATE affected 0 rows', { userId });
